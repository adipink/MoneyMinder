...
struct FilteredDetailView: View {
        ...
        NavigationView{
            ...
            ScrollView(.vertical, showsIndicators: false) {
                VStack(spacing: 15) {
                    ...
                    ExpenseCard(expenses: expenseViewModel.expenses,isFilter: true).environmentObject(expenseViewModel)
                    
                    customSegmentedControl().padding(.top)
                    
                    //MARK: Currently Filtered Date With Amount
                    VStack(spacing: 15){
                        Text(expenseViewModel.convertingdDateToString()).opacity(0.7)
                        
                        Text(expenseViewModel.convertExpensesToCurrency(expenses: expenseViewModel.expenses,type: expenseViewModel.tabName)).font(.title.bold()).opacity(0.9)
                    ...
                    ForEach(expenseViewModel.expenses.filter{
                        ...
                        expense in
                        TransactionCardView(expense: expense).environmentObject(expenseViewModel)
                    }
    ...
    func filterView()->some View{
        ZStack{
            Color.black.opacity(expenseViewModel.showFilterView ? 0.25 : 0).ignoresSafeArea()
            ...
            if expenseViewModel.showFilterView{
                VStack(alignment: .leading, spacing: 10){
                    ...
                    DatePicker("", selection: $expenseViewModel.starDate, in: Date.distantPast...Date(), displayedComponents: [.date])
                        .labelsHidden()
                ...
                .overlay(alignment: .topTrailing, content: {
                    Button{
                        ...
                        Image(systemName: "xmark.circle.fill").font(.title3).foregroundColor(.black).padding(5)
                    }
                })
                .padding()
            }
        ...
        HStack(spacing: 0) {
            ForEach([ExpenseType.income, ExpenseType.expense], id: \.rawValue){ tab in
                Text(tab.rawValue.capitalized)
                    ...
                    .background{
                        //MARK: With Matched Geomerty Effect
